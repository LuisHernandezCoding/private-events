class Profile < ApplicationRecord
  ...
  validates :first_name, :last_name, allow_blank: true,
                                     format: { with: /\A[a-zA-Z]+\z/, message: 'must be a valid name (letters only)' }

  validates :gender, inclusion: { in: %w[male female other not_say] }

  validates :title, inclusion: { in: ProfilesController.titles }
  ...
  validates :phone, allow_blank: true,
                    format: { with: /\A[2-9]\d{2}[1-9]\d{2}\d{4}\z/,
                              message: 'must be a valid phone number, like 555-555-5555' }
